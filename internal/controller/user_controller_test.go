...
var _ = Describe("User Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			Expect(createPasswordSecret(ctx, k8sClient, "default", "user-password", "password", "UserPassword1!")).To(Succeed())
			...
			resource := &harborv1alpha1.User{
				...
				Spec: harborv1alpha1.UserSpec{
					...
					PasswordSecretRef: corev1.SecretKeySelector{
						LocalObjectReference: corev1.LocalObjectReference{Name: "user-password"},
						Key:                  "password",
					},
				},
			}
		...
		AfterEach(func() {
			...
			secret = &corev1.Secret{}
			_ = k8sClient.Get(ctx, types.NamespacedName{Name: "user-password", Namespace: "default"}, secret)
			_ = k8sClient.Delete(ctx, secret)
		})
