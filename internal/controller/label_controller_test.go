...
var _ = Describe("Label Controller", func() {
	Context("When reconciling a resource", func() {
		...
		const adminSecretName = "harbor-admin-label"
		const connName = "harbor-conn-label"
		...
		BeforeEach(func() {
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodPost && r.URL.Path == "/api/v2.0/labels" {
					w.Header().Set("Location", "/api/v2.0/labels/9")
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &LabelReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}

			_, err := controllerReconciler.Reconcile(ctx, reconcile.Request{NamespacedName: typeNamespacedName})
			Expect(err).NotTo(HaveOccurred())

			out := &harborv1alpha1.Label{}
			Expect(k8sClient.Get(ctx, typeNamespacedName, out)).To(Succeed())
			Expect(out.Status.HarborLabelID).To(Equal(9))
