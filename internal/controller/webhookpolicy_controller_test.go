...
var _ = Describe("WebhookPolicy Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "webhook-policy"
		const adminSecretName = "harbor-admin-webhook"
		const connName = "harbor-conn-webhook"
		...
		BeforeEach(func() {
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodPost && r.URL.Path == "/api/v2.0/projects/42/webhook/policies" {
					w.Header().Set("Location", "/api/v2.0/projects/42/webhook/policies/7")
			...
			resource := &harborv1alpha1.WebhookPolicy{
				...
				Spec: harborv1alpha1.WebhookPolicySpec{
					...
					Targets: []harborv1alpha1.WebhookTargetSpec{
						{Type: "http", Address: "https://example.com/hook"},
					},
				},
			}
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &WebhookPolicyReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}

			_, err := controllerReconciler.Reconcile(ctx, reconcile.Request{NamespacedName: typeNamespacedName})
			Expect(err).NotTo(HaveOccurred())

			out := &harborv1alpha1.WebhookPolicy{}
			Expect(k8sClient.Get(ctx, typeNamespacedName, out)).To(Succeed())
			Expect(out.Status.HarborWebhookPolicyID).To(Equal(7))
