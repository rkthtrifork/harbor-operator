...
var _ = Describe("ScannerRegistration Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "scanner-reg"
		const adminSecretName = "harbor-admin-scanner"
		const connName = "harbor-conn-scanner"
		...
		BeforeEach(func() {
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodPost && r.URL.Path == "/api/v2.0/scanners" {
					w.Header().Set("Location", "/api/v2.0/scanners/uuid-1")
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &ScannerRegistrationReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}

			_, err := controllerReconciler.Reconcile(ctx, reconcile.Request{NamespacedName: typeNamespacedName})
			Expect(err).NotTo(HaveOccurred())

			out := &harborv1alpha1.ScannerRegistration{}
			Expect(k8sClient.Get(ctx, typeNamespacedName, out)).To(Succeed())
			Expect(out.Status.HarborScannerID).To(Equal("uuid-1"))
