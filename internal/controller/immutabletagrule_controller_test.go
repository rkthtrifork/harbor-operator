...
var _ = Describe("ImmutableTagRule Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "immutable-rule"
		const adminSecretName = "harbor-admin-immutable"
		const connName = "harbor-conn-immutable"
		...
		BeforeEach(func() {
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				switch {
				case r.Method == http.MethodPost && r.URL.Path == "/api/v2.0/projects/42/immutabletagrules":
					w.WriteHeader(http.StatusCreated)
					return
				case r.Method == http.MethodGet && r.URL.Path == "/api/v2.0/projects/42/immutabletagrules":
			...
			resource := &harborv1alpha1.ImmutableTagRule{
				...
				Spec: harborv1alpha1.ImmutableTagRuleSpec{
					...
					Action:         "immutable",
					Template:       "repoMatches",
					TagSelectors: []harborv1alpha1.ImmutableSelector{
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &ImmutableTagRuleReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}

			_, err := controllerReconciler.Reconcile(ctx, reconcile.Request{NamespacedName: typeNamespacedName})
			Expect(err).NotTo(HaveOccurred())

			out := &harborv1alpha1.ImmutableTagRule{}
			Expect(k8sClient.Get(ctx, typeNamespacedName, out)).To(Succeed())
			Expect(out.Status.HarborImmutableRuleID).To(Equal(9))
