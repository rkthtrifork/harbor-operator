...
var _ = Describe("ScanAllSchedule Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "scanall-schedule"
		const adminSecretName = "harbor-admin-scanall"
		const connName = "harbor-conn-scanall"
		...
		BeforeEach(func() {
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodPost && r.URL.Path == "/api/v2.0/system/scanAll/schedule" {
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &ScanAllScheduleReconciler{Client: k8sClient, Scheme: k8sClient.Scheme()}

			_, err := controllerReconciler.Reconcile(ctx, reconcile.Request{NamespacedName: typeNamespacedName})
			Expect(err).NotTo(HaveOccurred())

			out := &harborv1alpha1.ScanAllSchedule{}
