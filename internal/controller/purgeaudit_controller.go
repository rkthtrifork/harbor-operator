...
func (r *PurgeAuditScheduleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	hashInput := fmt.Sprintf("type=%s\ncron=%s\nretention=%d\ninclude=%s\ndry=%t",
		cr.Spec.Schedule.Type, cr.Spec.Schedule.Cron, cr.Spec.Parameters.AuditRetentionHour, cr.Spec.Parameters.IncludeEventTypes, cr.Spec.Parameters.DryRun)
	...
	if cr.Status.LastAppliedScheduleHash == "" {
		...
		markReady(&cr.Status.HarborStatusBase, cr.Generation, "Created", "Purge audit schedule created")
	...
	if changed := markReady(&cr.Status.HarborStatusBase, cr.Generation, "Reconciled", "Purge audit schedule reconciled"); changed {
		if err := r.Status().Update(ctx, &cr); err != nil {
			return ctrl.Result{}, err
		}
	}
