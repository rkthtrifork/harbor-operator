...
var _ = Describe("Purge Audit Schedule Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "purge-audit"
		const adminSecretName = "harbor-admin-purgeaudit"
		const connName = "harbor-conn-purgeaudit"
		...
		BeforeEach(func() {
			server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodPost && r.URL.Path == "/api/v2.0/system/purgeaudit/schedule" {
			...
			resource := &harborv1alpha1.PurgeAuditSchedule{
				...
				Spec: harborv1alpha1.PurgeAuditScheduleSpec{
					...
					Parameters: harborv1alpha1.PurgeAuditParameters{
						AuditRetentionHour: 720,
						IncludeEventTypes:  "create_artifact,delete_artifact",
						DryRun:             false,
					},
				},
			}
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &PurgeAuditScheduleReconciler{
				Client: k8sClient,
			...
			out := &harborv1alpha1.PurgeAuditSchedule{}
